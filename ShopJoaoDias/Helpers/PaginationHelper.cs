...
namespace ShopJoaoDias.Helpers
{
    ...
    {
        public CategoryProduct CategoryProduct(int totalProduct, int page, string q = "", List<ProductDO>? productDOs = null, int perPage = 12, int sort = 1)
        {
            var categoryProduct = new CategoryProduct();
            var pageSizeDots = Math.Ceiling(decimal.Parse(totalProduct.ToString()) / perPage);
            int pageCount = (int)Math.Round(pageSizeDots);
            ...
            if (pageCount > 1)
            {
                ...
                {
                    var active = i == page ? "is-active" : "";
                    pageHtml += "<li class='" + active + "'><a href='/Home/Search?q=" + q + "&p=" + i + "'>" + i + "</a></li>";
                }
